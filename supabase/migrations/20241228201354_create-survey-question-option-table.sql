-- Create the survey_question_option table with RLS
CREATE TABLE public.survey_question_option (
...
ALTER TABLE public.survey_question_option ENABLE ROW LEVEL SECURITY;

-- Create a policy that allows anyone to read the user table
CREATE POLICY "read_survey_question_option" ON public.survey_question_option
...
CREATE POLICY "insert_survey_question_option" ON public.survey_question_option
...
CREATE POLICY "delete_survey_question_option" ON public.survey_question_option
...
CREATE TRIGGER update_survey_question_option_modtime
  BEFORE UPDATE ON public.survey_question_option
  FOR EACH ROW
