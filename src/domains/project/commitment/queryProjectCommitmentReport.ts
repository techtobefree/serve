    ...
    queryFn: async () => {
      try {
        if (!projectEventId || (!attendeeSurveyId && !commitmentSurveyId)) {
          showToast("Missing project event ID or survey ID", {
            duration: 5000,
            isError: true,
          });
        ...
        const { data: surveys, error: surveyError } = await clientSupabase
          ...
          .in(
            "id",
            [attendeeSurveyId, commitmentSurveyId].filter(
              (i) => !!i
            ) as string[]
          );
        ...
        if (error) {
          showToast("Error getting commitments", {
            duration: 5000,
            isError: true,
          });
        ...
        db_commitments.forEach((commitment: any) => {
          compositeMap.set(commitment.created_by, {
            commitment,
            responses: {},
          });
        });
        ...
        responses.forEach((response: any) => {
          try {
            ...
            composite.responses[response.survey_question_id] =
        ...
        const commitments = db_commitments.map((item: any) => {
          const CommitmentStart = new TZDate(
            item.commitment_start
          ).withTimeZone(event.timezone);
          const CommitmentEnd = new TZDate(item.commitment_end).withTimeZone(
            event.timezone
          );
          ...
          const row: any = {
            ...
            Hours:
              (CommitmentEnd.getTime() - CommitmentStart.getTime()) /
              1000 /
              60 /
              60,
          ...
          Object.entries(compositeMap.get(item.created_by).responses).forEach(
            ([questionId, response]) => {
              try {
        ...
        downloadTextFile(
          `project_commitments_${event.project_event_date}.csv`,
          jsonToCsv(commitments)
        );
      } catch (e: any) {
        console.error("Error creating commitment report:", e);
        showToast(`Error creating commitment report. ${e.message as string}`, {
          duration: 5000,
          isError: true,
        });
      }
