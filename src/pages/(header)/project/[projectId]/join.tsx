...
export function ProjectJoinPageComponent({ userId }: Props) {
  ...
  const { data, isError, isLoading } = useJoinProjectByIdQuery(
    projectId,
    userId
  );
    ...
    <>
      ...
      <div className="flex justify-center">
        {userId ? (
          ...
          <div>You must login to follow a project.</div>
        )}
        <IonButton
          color="secondary"
          onClick={() => {
...
const ProjectJoinPage = observer(() => {
  return <ProjectJoinPageComponent userId={userStore.current?.id} />;
});
