...
const SURVEY_TYPE_MESSAGE: {
  ...
  The prompt will be used as column headers in reports.`,
  generic:
    "This survey will use the prompts as headers whenever generating a report.",
};
...
export default function SurveyEditPage() {
  ...
  const { surveyType }: { surveyType?: keyof typeof SURVEY_TYPE } =
    location.state || {};
  const { data: survey, isLoading, isError } = useSurveyByIdQuery(surveyId);
  const upsertSurvey = useUpsertSurvey(
    { surveyId: survey?.id },
    (error?: Error) => {
      if (!error) {
        navigate(-1);
