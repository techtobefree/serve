...
export function LayoutComponent({ userId }: Props) {
  ...
  if (
    ...
    (!profile?.handle || !profile.sensitive_profile[0].accepted_at)
  ) {
    // Sorry for this
    return (
      <div className="bg-[#f0f0f0]">
        <Profile userId={userId} canEdit={true} initial />
        <Toast />
      </div>
    );
  }
...
const Layout = observer(() => {
  return <LayoutComponent userId={userStore.current?.id} />;
});
