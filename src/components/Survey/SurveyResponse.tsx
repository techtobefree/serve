...
function isValidResponse(responses: MaybeResponse[]) {
  const missingResponse = responses.find(
    (response) => response.question.required && !response.response_text
  );
  if (missingResponse) {
    showToast(`${missingResponse.question.question_text} is required`, {
      duration: 5000,
      isError: true,
    });
    return false;
      ...
      <div className="flex flex-col gap-4">
        {responses.map((response, index) => {
          const ResponseComponent =
            QUESTION_MAP[response.question_type as keyof typeof QUESTION_TYPE]
              .response;
          return (
            <ResponseComponent
              key={index}
  ...
  ({
    ...
    return (
      <SurveyResponseComponent
        ...
        projectId={projectId}
        responses={[
          ...surveyStore.current.responses.map((i) => ({
            ...i,
