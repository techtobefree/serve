...
} from "../../domains/address/addressComponents";
...
export default function LocationPicker({
  ...
  return (
    <>
      ...
      <PlaceAutocomplete
        onPlaceSelect={({ address: newAddress, location, place }) => {
          if (location) {
            map?.panTo(location);
            map?.setZoom(17);
            changeLocation(location);
      ...
      <div className="h-[50vh] w-full">
        <Map
          ...
          defaultCenter={
            selectedLocation || {
              lat: 40.25511147870447,
              lng: -111.66301131248474,
            }
          }
          defaultZoom={13}
          onClick={(e) => {
            ...
            if (placeId) {
              places?.getDetails({ placeId }, (results, status) => {
                if (
                  ...
                  changeAddress(
                    decodeAddressComponents(results.address_components)
                  );
                } else {
              ...
              void new google.maps.Geocoder().geocode(
                ...
                (results, status) => {
                  if (status === google.maps.GeocoderStatus.OK && results) {
                    ...
                    const newAddress =
                      decodeAddressComponents(addressComponents);
                    changeAddress(newAddress);
