...
function ToastItem({
  ...
  if (typeof message === "string") {
    const classes = [
      "bg-gray-800 text-gray-200 rounded shadow overflow-hidden",
    ];
...
export function ToastComponent({ toasts }: Props) {
  return (
    toasts.length > 0 && (
      <div className={`z-50 fixed top-20 flex flex-col gap-2 right-2`}>
        {toasts.map((toast) => (
          <ToastItem
            key={toast.id}
            id={toast.id}
...
const Toast = observer(() => {
  return <ToastComponent toasts={[...toastStore.current]} />;
});
