  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      if (!value.name || !value.owner_id) {
        showToast("Name and Admin are required", {
          duration: 5000,
          isError: true,
        });
        return;
      ...
      if (res.error) {
        ...
        if (!project.id) {
          showToast("You can create a survey for attendees and/or volunteers", {
            duration: 5000,
          });
        }
      }
      ...
      <div>
          ...
          {(field) => (
            ...
            >
              ...
              <IonCheckbox
                ...
                onIonChange={(e) => {
                  field.handleChange(!e.detail.checked);
                }}
              />
            </IonItem>
          )}
            ...
            {(field) => (
              <IonItem>
                <UploadImage
                  ...
                  close={() => {
                    field.handleChange(
                      getPublicUrl(projectPicturePath(project.id as string))
                    );
                  }}
                />
              </IonItem>
        ...
        {!project.id && (
          <IonItem disabled>
            ...
            <span className="p-4">
              After creating the project, an image can be attached.
            </span>
          </IonItem>
        )}
